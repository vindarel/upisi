...
#+img:https://upload.wikimedia.org/wikipedia/commons/7/7f/Liferea-linux-060.png

# si on ne saute pas une ligne, l'image est ignorée. bug todo. autre bug: ce commentaire est compté dans la doc.

sudo aptitude install -y liferea


#: Devede, créer des DVDs
#+gui:toggle=True
#+img:im/gaston.jpg
sudo aptitude install -y devede
#+gui:toggle=False
...
sudo apt-get install -y  aptitude emacs foo third cmus-remote
